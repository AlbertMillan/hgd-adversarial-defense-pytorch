...
class Attacks:
    ...
    def __init__(self, model, eps, N_train, N_test, limits, momentum=None, is_normalized=False, retain=False):
        ...
        (self.max_val, self.min_val, self.eps_size) = limits
    ...
    def get_stored(self, mode):
        """ Get wether adversarial examples have been stored. """
        return self.adv_stored[mode]
    
    
    def set_stored(self, mode, is_stored):
        """ Confirm adversarial examples have been stored."""
    ...
    def retain_adversaries(self, x_adv, y_batch, mode):
        """ Store adversarial images to array to retrieve on later iterations. """
        if self.retain:
    ...
    def compute_alpha(self, eps, max_iter, is_normalized):
        ...
        Output:
            - alpha:
                - (is_normalized : True)  := cuda tensor of shape [1,C,1,1], containing alpha values for each channel C
                - (is_normalized : False) := scalar alpha
        """
        alpha = None
        if is_normalized:
            # Epsilon is in the range of possible inputs.
            # Reshape to  [1,C,1,1] to enable broadcasting
            alpha = (eps * self.eps_size)[np.newaxis,:,np.newaxis,np.newaxis]
    ...
    def clamp_tensor(self, x, is_normalized):
        ...
        if is_normalized:
            x.data[:,0,:,:].clamp_(min=self.min_val[0], max=self.max_val[0])
            x.data[:,1,:,:].clamp_(min=self.min_val[1], max=self.max_val[1])
            x.data[:,2,:,:].clamp_(min=self.min_val[2], max=self.max_val[2])
        else:
    ...
    def fast_pgd(self, x_batch, y_batch, max_iter, mode):
        ...
        if mode == 'train':
            eps = (torch.randint(low=1, high=17, size=(x_batch.size(0),))).float().cuda() / 255.
        elif mode == 'test':
            eps = self.eps
        
        
        # Compute alpha. Alpha might vary depending on the type of normalization.
        alpha = self.compute_alpha(eps, max_iter, self.is_normalized)
